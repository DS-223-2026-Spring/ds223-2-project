...
`docker compose up` brings up the following services (order matters for **`back`** / **`app`**):
...
- **docker-build** – creates a throwaway **`.env`** in the job (placeholders for Compose), validates Compose, and builds **back**, **app**, and **etl_db** images (ETL is built but not executed in CI).
